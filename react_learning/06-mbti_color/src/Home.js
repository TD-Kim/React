...
function Home() {
  ...
  useEffect(() => {
    async function handleScroll() {
      ...
      // 여기서 잠깐, 한 가지 주의할 점이 있습니다. 이 함수는 언제 실행되어야 할까요? 일단 다음 페이지가 있는 경우에만 실행해야 하고(nextPageRef.current값이 존재할 때만 실행)
      // handleScroll()이 실행되지 않는 경우에만 실행해야 합니다. 이게 무슨 말이냐면, 스크롤 이벤트는 스크롤 할 때마다 계속 실행되기 때문에 중복해서 실행될 수
      // 있습니다. 그래서 isLoadingRef라는 Ref Object를 만들어서 중복으로 실행되지 않도록 막아주겠습니다.
      const maxScrollTop =
